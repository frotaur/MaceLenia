...
def run_intrinsic_evo(model: MaCELenia, num_runs=150, default=True, sense=True, extra_string=""):
    ...
    for k in tqdm(range(num_runs)):
        ...
        model.b = 6.0  # Set Beta to 6.0 (can modify)

        init_mass = model.state.sum()
        dead_mass = (
            0.06 * init_mass
        )  # Define the model as dead if it retains less than 6% of its initial mass (can modify for how aggressive you want the evolution to be)
...
model = MaCELenia(
    size=(1, 800, 800), dt=0.1, num_channels=3, device="cuda", has_food=True, save_dir="intrinsic"
)
