...
def perlin(shape: tuple, wavelengths: tuple, num_channels=3, black_prop: float = 0.3, device="cpu"):
    ...
    fp = FractalPerlin2D((B * num_channels, H_new, W_new), [frequency], [1 / 0.7053], generator=gen)()[
        :, :H, :W
    ].reshape(B, num_channels, H, W)  # (B,C,H,W) noise)

    return torch.clamp((fp + (0.5 - black_prop) * 2) / (2 * (1.0 - black_prop)), 0, 1)


def perlin_fractal(
    shape: tuple, max_wavelength: int, persistence=0.5, num_channels=3, black_prop: float = 0.3, device="cpu"
):
    """
    Makes fractal noise with dominant wavelength equal to max_wavelength.
    """
    ...
    return (
        ...
        * sum(
            [
                ...
                * perlin(
                    shape,
                    [int(2 ** (-i) * max_wavelength)] * 2,
                    black_prop=black_prop,
