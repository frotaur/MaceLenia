...
def sobel_y(x):
    k_y = torch.tensor([[-1, 0, 1],
                        [-2, 0, 2],
                        [-1, 0, 1]],
                       dtype=torch.float32, device="cuda:0").T.tile((x.shape[1],1, 1, 1))
...
class FlowLenia(MCLenia):
    ...
    def get_string_state(self):
        return f"total mass: {self.state.sum().item():.2f}, temp: {self.theta_x:.2f}, sigma: {self.sigma_rt:.2f}"
    ...
    def random_food_chan(self, num_spots=100, food_size=5):
        ...
        """
        places = [[random.randint(food_size, self.h - food_size), random.randint(food_size, self.w - food_size)] for _
                  in
                  range(num_spots)]
